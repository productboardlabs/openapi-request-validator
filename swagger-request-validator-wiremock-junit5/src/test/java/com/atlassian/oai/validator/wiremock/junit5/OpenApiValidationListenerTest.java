...
public class OpenApiValidationListenerTest {
    ...
    void shouldDetectInvalidRequestInCaseOfOas3() {
        final List<ValidationReport.Message> actualMessages =
                testValidationException(OAS3_FILE, INVALID_API_PATH, VALID_RESPONSE_BODY);
        ...
        assertThat(actualMessages.get(0).getMessage(),
    ...
    void shouldDetectInvalidRequestInCaseOfOas2() {
        final List<ValidationReport.Message> actualMessages =
                testValidationException(OAS2_FILE, INVALID_API_PATH, VALID_RESPONSE_BODY);
    ...
    void shouldDetectInvalidResponseInCaseOfOas3() {
        final List<ValidationReport.Message> actualMessages =
                testValidationException(OAS3_FILE, VALID_PATH, INVALID_RESPONSE_BODY);
        ...
        assertThat(actualMessages.get(1).getMessage(),
    ...
    void shouldDetectInvalidResponseInCaseOfOas2() {
        final List<ValidationReport.Message> actualMessages =
                testValidationException(OAS2_FILE, VALID_PATH, INVALID_RESPONSE_BODY);
    ...
    private List<ValidationReport.Message> testValidationException(final String specFile,
                                                                    final String requestPath,
                                                                    final String responseBody) {
        testEndpoint(specFile, requestPath, responseBody);

        await().pollDelay(Duration.ofMillis(100))
                .until(() -> !VALIDATION_LISTENER.getReport().getMessages().isEmpty());
    ...
    private void createStub(final String spec, final String path, final String responseBody) {
        WIREMOCK.stubFor(get(urlEqualTo(path))
                .withServeEventListener("open-api-validation-listener",
                        new OpenApiValidationListener.OasUrlParameter(spec))
